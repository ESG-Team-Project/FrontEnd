...
export default function ProtectedRoute({ children, redirectTo = '/login' }: ProtectedRouteProps) {
  ...
  const queryBypass =
    process.env.NODE_ENV !== 'production' && searchParams.get('bypass_auth') === 'true';
  ...
  console.log(
    `ProtectedRoute Render Check on ${pathname}: isMyPage=${isMyPage}, shouldBypass=${shouldBypass}, isInitialized=${isInitialized}, isLoggedIn=${isLoggedIn}`
  );

  // 리다이렉션 로직: 초기화 완료 후 상태 기반으로 실행
  useEffect(() => {
    console.log(
      `ProtectedRoute Effect Check on ${pathname}: isInitialized=${isInitialized}, isLoggedIn=${isLoggedIn}, shouldRedirect=${!isMyPage && !shouldBypass && isInitialized && !isLoggedIn}`
    );
  ...
  if (!isInitialized) {
    console.log(
      `ProtectedRoute Render on ${pathname}: Showing Loading Indicator (Not Initialized).`
    );
  ...
  if (isInitialized && isLoggedIn) {
    console.log(
      `ProtectedRoute Render on ${pathname}: Rendering children (Initialized and Logged In).`
    );
    return <MyErrorBoundary>{children}</MyErrorBoundary>;
  }
  ...
  console.log(
    `ProtectedRoute Render on ${pathname}: Rendering null (Initialized but Not Logged In - waiting for redirect effect).`
  );
  return null;
}
