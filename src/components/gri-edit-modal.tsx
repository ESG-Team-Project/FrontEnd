...
export function GriEditModal({ isOpen, onOpenChange, editingCategory, onSave }: GriEditModalProps) {
  ...
  const handleModalTimeSeriesChange = (index: number, field: keyof TimeSeriesDataPoint, value: string | number) => {
  ...
  const createTimeSeriesPoint = (format: 'year-only' | 'with-quarter' | 'with-month'): TimeSeriesDataPoint => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg md:max-w-2xl lg:max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>
            {editingCategory?.category.name} 데이터 편집
          </DialogTitle>
        </DialogHeader>
        ...
        {editingCategory && (
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-2 border-b">
            ...
            <Select 
              value={dataType} 
              onValueChange={(value: GRIDataType) => 
                handleModalDataTypeChange(value)
              }
            >
        ...
        {/* 저장 메시지 표시 */}
        {saveMessage && (
          <div className={`p-3 mb-2 rounded text-sm ${saveMessage.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            {saveMessage.text}
          </div>
        )}
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex gap-2">
                        <Input
                          ...
                          step={modalDecimalPlaces > 0 ? `0.${'0'.repeat(modalDecimalPlaces-1)}1` : '1'}
                          placeholder="숫자 값 입력"
                          className="flex-1"
                        />
                        <Input
                  ...
                  <div className="py-2">
                    ...
                    <div className="overflow-x-auto -mx-3 px-3 pb-2">
                      {modalTimeSeriesData.length > 0 ? (
                            ...
                            <Table className="border">
                              <TableHeader className="bg-muted/50">
                                <TableRow>
                                  ...
                                  <TableHead className="font-semibold text-center w-14">삭제</TableHead>
                                </TableRow>
                              </TableHeader>
                              <TableBody>
                                {modalTimeSeriesData.map((point, index) => (
                                  <TableRow key={point.id}>
                                    <TableCell>
                                      <Input type="number" value={point.year ?? ''} onChange={(e) => handleModalTimeSeriesChange(index, 'year', e.target.value)} className="w-full" placeholder="YYYY"/>
                                    </TableCell>
                                    <TableCell>
                                      <Input 
                                        type="number" 
                                        value={point.quarter ?? ''} 
                                        onChange={(e) => handleModalTimeSeriesChange(index, 'quarter', e.target.value)} 
                                        className="w-full" 
                                        placeholder="1-4" 
                                        min="1" 
                                        max="4"
                                    ...
                                    <TableCell>
                                      <Input 
                                        type="number" 
                                        value={point.month ?? ''} 
                                        onChange={(e) => handleModalTimeSeriesChange(index, 'month', e.target.value)} 
                                        className="w-full" 
                                        placeholder="1-12" 
                                        min="1" 
                                        max="12"
                                    ...
                                    <TableCell>
                                      <Input
                                         type={editingCategory?.category.isQuantitative ? "number" : "text"}
                                         value={point.value ?? ''}
                                         onChange={(e) => handleModalTimeSeriesChange(index, 'value', e.target.value)}
                                         className="w-full"
                                         step={editingCategory?.category.isQuantitative ? "any" : undefined}
                                         placeholder="값 입력"
                                       />
                                    </TableCell>
                                     <TableCell>
                                        <Input value={point.unit ?? ''} onChange={(e) => handleModalTimeSeriesChange(index, 'unit', e.target.value)} className="w-full" placeholder="단위"/>
                                    </TableCell>
                                    <TableCell>
                                        <Input value={point.notes ?? ''} onChange={(e) => handleModalTimeSeriesChange(index, 'notes', e.target.value)} className="w-full" placeholder="비고"/>
                                    </TableCell>
                                    <TableCell className="text-center">
                                        <CustomButton type="button" variant="ghost" size="icon" onClick={() => removeTimeSeriesDataPoint(point.id)}>
                          ...
                          <p className="text-sm text-muted-foreground text-center py-4">추가할 데이터 항목이 없습니다.</p>
                      )}
                    </div>
                  </div>
                );
        ...
        <DialogFooter className="pt-2 border-t">
          <div className="flex items-center gap-2 w-full justify-between">
            <DialogClose asChild>
              <CustomButton type="button" variant="secondary">취소</CustomButton>
            </DialogClose>
            <div className="flex items-center gap-2">
              {isSaving ? (
                <div className="flex items-center gap-2">
