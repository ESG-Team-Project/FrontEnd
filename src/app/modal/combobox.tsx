...
const esgIndicators = {
  environment: [
    ...
    },
    {
      id: "304-2",
      label:
        "생태계 보호지역/주변지역에 사업활동, 제품, 서비스 등으로 인한 영향",
    },
    ...
    { id: "306-5", label: "폐기물 매립" },
    {
      id: "307-1",
      label:
        "환경법 및 규정 위반으로 부과된 중요한 벌금의 액수 및 비금전적 제재조치의 수",
    },
    { id: "308-1", label: "환경 기준 심사를 거친 신규 공급업체" },
    {
      id: "308-2",
      label:
        "공급망 내 실질적 또는 잠재적인 중대한 부정적 환경영향 및 이에 대한 조치",
    },
  ],
  social: [
...
export function ESGCombobox() {
  ...
  const [category, setCategory] = React.useState<EsgCategory | null>(null);
                ...
                {categoryOptions.map((option) => (
                  <CommandItem
                    ...
                    onSelect={(value) => {
                      ...
                      setOpenCategoryPopover(false); // Close category popover
                      setOpenIndicatorPopover(true); // Open indicator popover
                    }}
                  >
                    {option.label}
                  </CommandItem>
